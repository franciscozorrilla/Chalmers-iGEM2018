function proliferation = p28_action(p28_0, mpf_peak, tf)
...
% Concentrations in microM
    ...
    % Fluctuations in the experienced p28 concentration
    uptake_ratio = normrnd(1.0, 0.5); % Optimera sd?
    if uptake_ratio < 0 % Probability for this should be small (less than 0.15% (now it's a bit more))
        uptake_ratio = 1.0; 
    end
    %uptake_ratio = lognrnd(0, 0.25);
    x0(7) = uptake_ratio * x0(7);

    % Fluctuations in uptake rate
    k_uptake_mean = 0.1326;
    k_uptake = normrnd(k_uptake_mean, k_uptake_mean / 2);
    if k_uptake < 0 % Probability for this should be small (less than 3%)
        k_uptake = k_uptake_mean; 
    end
    %k_uptake = lognrnd(0, 1) * k_uptake_mean;
    
    % Solve system
    proliferation = 1;
    tI = [0, tf];
    [t, x] = ode23s(@(t, x) f_p28_action(t, x, k_uptake), tI, x0);
