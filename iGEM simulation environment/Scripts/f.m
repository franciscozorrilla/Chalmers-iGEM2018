...
for organism = 1:length(superModel.organismID)
    for upMet = 1:length(Vmax)
        if Vmax(upMet,organism)==0 & Ks(upMet,organism) ==0
            ...
            bound(upMet,organism) = (Vmax(upMet,organism) .* x(upMet)) ./ (Ks(upMet,organism) + x(upMet));
        end
    end
end
...
if isempty(solSbo.f) & solSbo.skip ~= 1
...
dx(19)= - abs(solSbo.x(1890)*x(1)) - abs(solCan.x(4007)*x(5)) - abs(solCol.x(3124)*x(6)) - D*(x(19) -  Wf);

%Methionine mass balance
dx(20)= - abs(solSbo.x(1884)*x(1)) - abs(solCan.x(4004)*x(5)) - abs(solCol.x(3121)*x(6)) - D*(x(20) - Mf);
